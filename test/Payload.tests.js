...
var Base = require("./BaseTest.js.example");
...
describe("PushPayload test", function () {
	...
	it("message test2", function (done) {
		...
		var payload = client
			.push()
			.setMessage("msg content", "msg title", "content type", Base.EXTRAS)
			.setOptions(123456);
		payload.toJSON().should.equal(result);
		done();
	});
	...
	it("notification test2", function (done) {
		...
		var payload = client
			.push()
			.setNotification(Base.ALERT, JPush.ios(Base.ALERT, "happy", 5))
			.setOptions(123456);
		payload.toJSON().should.equal(result);
		done();
	});
	...
	it("notification test3", function (done) {
		...
		var payload = client
			...
			.setNotification(
				Base.ALERT,
				JPush.ios(Base.ALERT, "happy", 2, true, Base.EXTRAS),
